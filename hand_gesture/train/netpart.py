...
#		inp_holder = tf.image.random_brightness(inp_holder,50)
		...
		l3=mod.NIN(3,312,312,activation=M.PARAM_ELU)
...
cat_labholder = tf.placeholder(tf.float32,[None,None,None,3])

feature = build_model(inpholder)
bias,conf,cat =tf.split(feature,[4,1,3], 3)
...
conf_loss = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(logits=conf,targets=c_labholder,pos_weight=1))
cat_loss = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(logits=cat,targets=cat_labholder,pos_weight=1))

total_loss = 8*(bias1_loss+ bias2_loss) + conf_loss + cat_loss
step = tf.train.AdamOptimizer(0.0001).minimize(total_loss)
