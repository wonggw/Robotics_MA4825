...
def write_to_file(annotation_path,image_number, xywh_list,image_format):
    ...
    if xywh_list ==[]:
        ...
        new_data = "\t".join([image_path]+xywh_list+[str(category)])+ "\n"
...
def read_from_file(annotation_path,image_number,image_format):
    ...
    with open(annotation_path, 'r') as fp:
        for i, line in enumerate(fp):
            if i == image_number-1:
                data = line.strip()
                data = data.split(image_format)   #'abc' , ''"
...
working_path = '/home/kyubey/Desktop/MA4825/hand_gesture/label/record_video/imgs_7' #Folder which contains the images
image_format = 'png' #Image format of the images inside the folder specified in working path
#Initialisation
