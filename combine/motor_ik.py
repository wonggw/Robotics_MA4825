...
for i in range(1000):
	dxl_io.set_moving_speed({motor_ids[0]:150})
	dxl_io.set_goal_position({motor_ids[0]: 0})
	dxl_io.set_moving_speed({motor_ids[2]:150})
	dxl_io.set_goal_position({motor_ids[2]: 0})
	dxl_io.set_moving_speed({motor_ids[1]:150})
	dxl_io.set_goal_position({motor_ids[1]: 0})
time.sleep(1)
...
while True:
	...
	if coords:
		for coord in coords:
			...
			arm.ee = [(x_hand-320)/3200, (y_hand-240)/2400-0.1,z_hand/100]
