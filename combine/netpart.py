...
def build_model(inp_holder):
	with tf.variable_scope('YOLO_V0'):
		inp_holder = tf.image.random_saturation(inp_holder,lower=0.8,upper=1.2)
		inp_holder = tf.image.random_contrast(inp_holder,lower=0.8,upper=1.2)
...
cat_labholder = tf.placeholder(tf.float32,[None,None,None,4])

feature = build_model(inpholder)
bias,conf,cat =tf.split(feature,[4,1,4], 3)
...
with tf.variable_scope('Total_losses'):
	total_loss = (bias1_loss+ bias2_loss) + conf_loss + cat_loss
